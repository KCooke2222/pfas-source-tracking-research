...
export default function SimpleDataTool({ analysisMode = "1633_pfas" }) {
  ...
  useEffect(() => {
    if (!nmds && !loading) {
      setLoading(true);
      axios
        .get(`${apiBase}/base-nmds?mode=${analysisMode}`)
        .then((res) => {
  ...
  useEffect(() => {
    if (mode !== "demo") return;
    axios
      .get(`${apiBase}/demo/options?mode=${analysisMode}`)
      .then((res) => {
  ...
  const resetToBase = async () => {
    ...
    try {
      const res = await axios.get(`${apiBase}/base-nmds?mode=${analysisMode}`);
      setNmds(res.data.nmds);
    } catch (e) {
  ...
  const handleRunDemo = async () => {
    ...
    setLoading(true);
    try {
      const res = await axios.post(`${apiBase}/demo/run`, {
        name: demoName,
        mode: analysisMode,
      });
      setPreview(res.data.preview);
  ...
  const handleDownload = async () => {
    ...
    if (!nmds.new_points || nmds.new_points.length === 0) {
      setError(
        "No new data points to download. Upload or run demo data first."
      );
      return;
    }
    ...
    try {
      ...
      let filename = `${analysisMode}_nmds_results_${
  ...
  const handleDownloadTemplate = () => {
    ...
    link.href = `${apiBase}/template?mode=${analysisMode}`;
    link.download = `${analysisMode}_template.csv`;
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-primary-600 mt-1">
              {analysisMode === "1633_pfas"
                ? "1633 PFAS Compounds Analysis"
                : "Diagnostic Target & Suspect Chemicals Analysis"}
            </p>
          </div>
          {loading && <Spinner />}
        </div>
        ...
        {!preview && !nmds && !loading && (
          <div className="text-center text-primary-500 mt-12 py-12">
            ...
            <p className="mt-4 text-lg">
              Upload a CSV file or choose demo data to begin analysis.
            </p>
          </div>
        )}
      </div>
